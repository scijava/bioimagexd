...
#
SET(LIBRARY_OUTPUT_PATH ${VTKBXD_BINARY_DIR}/bin CACHE PATH
    "Single output directory for building all libraries.")
SET(EXECUTABLE_OUTPUT_PATH ${VTKBXD_BINARY_DIR}/bin CACHE PATH
...
# Copy the CMake option to a setting with VTKBXD_ prefix for use in
# our project.  This name is used in vtkmyConfigure.h.in.
SET(VTKBXD_BUILD_SHARED_LIBS ${BUILD_SHARED_LIBS})
...
IF (VTK_WRAP_TCL)
  ...
  OPTION(VTKBXD_WRAP_TCL
         "Wrap classes into the TCL interpreted language."
         ON)

  IF(VTKBXD_WRAP_TCL)
    INCLUDE(${VTK_CMAKE_DIR}/vtkWrapTcl.cmake)
  ENDIF(VTKBXD_WRAP_TCL)

ELSE (VTK_WRAP_TCL)

  IF (VTKBXD_WRAP_TCL)
    MESSAGE("Warning. VTKBXD_WRAP_TCL is ON but the VTK version you have "
            "chosen has not support for Tcl (VTK_WRAP_TCL is OFF).  "
            "Please set VTKBXD_WRAP_TCL to OFF.")
    SET (VTKBXD_WRAP_TCL OFF)
  ENDIF (VTKBXD_WRAP_TCL)
...
IF (VTK_WRAP_PYTHON)
  ...
  OPTION(VTKBXD_WRAP_PYTHON
         "Wrap classes into the Python interpreted language."
         ON)

  IF (VTKBXD_WRAP_PYTHON)
    INCLUDE(${VTK_CMAKE_DIR}/vtkWrapPython.cmake)
    IF (WIN32)
      IF (NOT BUILD_SHARED_LIBS)
        MESSAGE(FATAL_ERROR "Python support requires BUILD_SHARED_LIBS to be ON.")
        SET (VTKBXD_CAN_BUILD 0)
      ENDIF (NOT BUILD_SHARED_LIBS)
    ENDIF (WIN32)
  ENDIF (VTKBXD_WRAP_PYTHON)

ELSE (VTK_WRAP_PYTHON)

  IF (VTKBXD_WRAP_PYTHON)
    MESSAGE("Warning. VTKBXD_WRAP_PYTHON is ON but the VTK version you have "
            "chosen has not support for Python (VTK_WRAP_PYTHON is OFF).  "
            "Please set VTKBXD_WRAP_PYTHON to OFF.")
    SET (VTKBXD_WRAP_PYTHON OFF)
  ENDIF (VTKBXD_WRAP_PYTHON)
...
IF (VTK_WRAP_JAVA)
  ...
  OPTION(VTKBXD_WRAP_JAVA
         "Wrap classes into the Java interpreted language."
         ON)

  IF (VTKBXD_WRAP_JAVA)
    SET(VTK_WRAP_JAVA3_INIT_DIR "${VTKBXD_SOURCE_DIR}/Wrapping")
    ...
    SET(VTK_JAVA_HOME ${VTKBXD_BINARY_DIR}/java/vtkmy)
    MAKE_DIRECTORY(${VTK_JAVA_HOME})
  ENDIF (VTKBXD_WRAP_JAVA)

ELSE (VTK_WRAP_JAVA)

  IF (VTKBXD_WRAP_JAVA)
    MESSAGE("Warning. VTKBXD_WRAP_JAVA is ON but the VTK version you have "
            "chosen has not support for Java (VTK_WRAP_JAVA is OFF).  "
            "Please set VTKBXD_WRAP_JAVA to OFF.")
    SET (VTKBXD_WRAP_JAVA OFF)
  ENDIF (VTKBXD_WRAP_JAVA)

ENDIF (VTK_WRAP_JAVA)

# Setup our local hints file in case wrappers need them.
SET(VTK_WRAP_HINTS ${VTKBXD_SOURCE_DIR}/Wrapping/hints)
