...
class ParticleReader:
    ...
    def read(self):
        ...
        for obj, sizemicro,size,cog,umcog,avgint in self.rdr:
            try:
                size = int(size)
            except:
                continue
...
class ParticleTracker:
    ...
    def readFromFile(self, filename):
        ...
        self.particles = []
        n = 999
        print "Reading from file base='",filename,"'"
        for i in range(0,n+1):
            file="%s_%d.csv"%(filename,i)            
            if os.path.exists(file):
                print "Reading from ",file
                reader = ParticleReader(file, i-1, filterObjectSize = self.filterObjectSize)
                self.particles.append(reader.read())
            elif i!=0:
                return
    ...
    def getStats(self):
        ...
        for i,pts in enumerate(self.particles):
            ...
            mindist=999999999
            maxdist=0
            print "There are %d particles in timepoint %d"%(i,len(pts))
            for particle in pts:
    ...
    def track(self):
        ...
        for tp in range(totalTimepoints):            
            ...
            for i,particle in enumerate(self.particles[tp]):
                # If particle is in track, just let the user know
                if particle.inTrack:
                    print "\nParticle %d / %d in timepoint %d is already in track %d"%(i,len(self.particles[tp]),tp,particle.trackNum)
                else:
                    print "\nTracking particle %d / %d in timepoint %d"%(i,len(self.particles[tp]),tp)
