...
::GenerateData()
{
  ...
  expFilter->Update();
  RealImageType* expOutput = expFilter->GetOutput();

  /**
   * Set bias field as second output
   */
  this->SetNthOutput(1, expFilter->GetOutput());
  ...
  divider->Update();
  OutputImageType* divOutput = divider->GetOutput();
...
template<class TInputImage, class TMaskImage, class TOutputImage>
typename N3MRIBiasFieldCorrectionImageFilter<TInputImage, TMaskImage, TOutputImage>::RealImageType *
N3MRIBiasFieldCorrectionImageFilter<TInputImage, TMaskImage, TOutputImage>
::GetBiasField()
{
  return dynamic_cast<RealImageType*>(this->ProcessObject::GetOutput(1));
}
