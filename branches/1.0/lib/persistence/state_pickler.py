...
#                    Numeric.ArrayType: self._do_numeric,
    ...
    def _do_numeric(self, value):
        idx = self._register(value)
        data = gzip_string(Numeric.dumps(value)).encode('base64')
...
class StateSetter:
    ...
    def _update_and_check_state(self, obj, state):
        ...
        if (metadata['class_name'] != cls.__name__):
            raise StateSetterError, \
                  'Instance and state do not have the same class name!'
        if (metadata['module'] != cls.__module__):
            raise StateSetterError, \
                  'Instance and state do not have the same module name!'
...
def _get_file_read(f):
    if hasattr(f, 'read'):
        return f
    elif type(f) in types.StringTypes:
        return open(f, 'rb')
    else:
