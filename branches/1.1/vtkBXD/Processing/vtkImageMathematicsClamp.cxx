/*=========================================================================
  ...
  Program:   BioImageXD
  Module:    $RCSfile: vtkImageMathematicsClamp.cxx,v $

  Edited by Lassi Paavolainen to include clamping of overflow (not everywhere)
...
=========================================================================*/
#include "vtkImageMathematicsClamp.h"
...
vtkStandardNewMacro(vtkImageMathematicsClamp);

//----------------------------------------------------------------------------
vtkImageMathematicsClamp::vtkImageMathematicsClamp()
{
...
int vtkImageMathematicsClamp::RequestInformation (
...
void vtkImageClamp(TValue &value, TIvar ivar, vtkImageData *data)
{
...
template <class T>
void vtkImageMathematicsClampExecute1(vtkImageMathematicsClamp *self,
                                      ...
                                      int outExt[6], int id)
{
  ...
  vtkImageClamp(constantk, self->GetConstantK(), in1Data);
  vtkImageClamp(constantc, self->GetConstantC(), in1Data);
...
template <class T>
void vtkImageMathematicsClampExecute2(vtkImageMathematicsClamp *self,
                                      vtkImageData *in1Data, T *in1Ptr,
    ...
    for (idxY = 0; !self->AbortExecute && idxY <= maxY; idxY++)
      ...
      for (idxR = 0; idxR < rowLength; idxR++)
        ...
        switch (op)
          {
          case VTK_ADD:
			val = static_cast<double>(*in1Ptr) + *in2Ptr;
			if (clamp)
			  {
				if (val > typeMax)
				  {
          ...
          case VTK_DIVIDE:
            if (*in2Ptr)
              ...
              if ( divideByZeroToC )
                {
                val = static_cast<double>(constantc);
                }
              else
                {
                val = static_cast<double>(outData->GetScalarTypeMax());
                }
              }
			if (clamp)
			  {
          ...
          case VTK_ATAN2:
              if (*in1Ptr == 0.0 && *in2Ptr == 0.0)
                ...
                {
                val =  static_cast<double>(atan2(static_cast<double>(*in1Ptr),
...
void vtkImageMathematicsClamp::ThreadedRequestData(
  vtkInformation * vtkNotUsed( request ),
  ...
  int outExt[6], int id)
{
  ...
  if (this->Operation == VTK_ADD || this->Operation == VTK_SUBTRACT ||
    ...
    if (!inData[1] || ! inData[1][0])
      {
      vtkErrorMacro(
        "ImageMathematicsClamp requested to perform a two input operation "
        "with only one input\n");
      return;
      }
    ...
    switch (inData[0][0]->GetScalarType())
      {
      vtkTemplateMacro(
        vtkImageMathematicsClampExecute2(this,inData[0][0],
    ...
    switch (inData[0][0]->GetScalarType())
      {
      vtkTemplateMacro(
        vtkImageMathematicsClampExecute1(this, inData[0][0],
                                         static_cast<VTK_TT *>(inPtr1),
...
//----------------------------------------------------------------------------
int vtkImageMathematicsClamp::FillInputPortInformation(
...
//----------------------------------------------------------------------------
void vtkImageMathematicsClamp::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  os << indent << "ClampOverflow: " << this->ClampOverflow << "\n";
}
