...
// Fixed need_close_tag flag in FlattenElement
// Otherwise vtkExtXMLUtilities.cxx is same as original vtkXMLUtilities.cxx
// except some minor cosmetic changes
...
typedef vtkstd::vector<vtkXMLDataElement*> vtkExtXMLUtilitiesDataElementContainer;

vtkStandardNewMacro(vtkExtXMLUtilities);
vtkCxxRevisionMacro(vtkExtXMLUtilities, "$Revision: 1.10 $");
...
//----------------------------------------------------------------------------
inline int vtkExtXMLUtilitiesEncodeEntities(unsigned char c, ostream &output)
{
  switch (c)
    {
    case '&':
...
//----------------------------------------------------------------------------
void vtkExtXMLUtilities::EncodeString(const char *input, int input_encoding, 
                                   ...
                                   int special_entities)
{
  ...
  if (no_input_encoding || no_output_encoding)
    ...
    while (*str)
      {
      if (!vtkExtXMLUtilitiesEncodeEntities(*str, output))
        {
        output << *str;
        }
      str++;
  ...
  if (output_encoding == VTK_ENCODING_UTF_8)
    ...
    if (input_encoding == VTK_ENCODING_US_ASCII || from_iso_8859)
      ...
      while (*str)
        {
        if (!special_entities || !vtkExtXMLUtilitiesEncodeEntities(*str, output))
          {
          if (*str > 0x7F)
            {
#if 0
...
//----------------------------------------------------------------------------
void vtkExtXMLUtilities::CollateAttributes(vtkXMLDataElement *elem, 
                                        ostream &os,
                                        const char *sep)
{
  if (!elem)
  ...
  for (i = 0; i < nb; i++)
    ...
    if (name)
      ...
      if (value)
        ...
        os << name << "=\"";
        vtkExtXMLUtilities::EncodeString(
...
//----------------------------------------------------------------------------
void vtkExtXMLUtilities::FlattenElement(vtkXMLDataElement *elem, 
                                     ostream &os,
                                     vtkIndent *indent,
                                     int indent_attributes)
{
  ...
  if (elem->GetNumberOfAttributes())
    ...
    if (indent && indent_attributes)
      ...
      memset(sep + 1, ' ', len);
      sep[len + 1] = '\0';
      vtkExtXMLUtilities::CollateAttributes(elem, os, sep);
      delete [] sep;
      }
    else
      {
      vtkExtXMLUtilities::CollateAttributes(elem, os);
      }
    }

  const char *cdata = elem->GetCharacterData();
  int nb_nested = elem->GetNumberOfNestedElements();
  int need_close_tag = (nb_nested || (cdata && strlen(cdata) != 0));
  ...
  if (nb_nested)
    ...
    for (int i = 0; i < nb_nested; i++)
      ...
      if (indent)
        ...
        vtkExtXMLUtilities::FlattenElement(elem->GetNestedElement(i), 
                                        os, &next_indent);
        }
      else
        {
        vtkExtXMLUtilities::FlattenElement(elem->GetNestedElement(i), os);
        }
      }
    if (indent)
      {
...
//----------------------------------------------------------------------------
int vtkExtXMLUtilities::WriteElementToFile(vtkXMLDataElement *elem, 
                                        const char *filename,
                                        vtkIndent *indent)
{
  if (!elem || !filename)
  ...
  vtkExtXMLUtilities::FlattenElement(elem, os, indent);
...
//----------------------------------------------------------------------------
vtkXMLDataElement*
vtkExtXMLUtilities::ReadElementFromStream(istream &is, int encoding)
{
  ...
  if (xml_parser->Parse())
    ...
    vtkExtXMLUtilities::UnFactorElements(res);
    }
...
//----------------------------------------------------------------------------
vtkXMLDataElement*
vtkExtXMLUtilities::ReadElementFromString(const char *str, int encoding)
{
  if (!str)
    {
    return 0;
  ...
  vtkXMLDataElement *res = 
    vtkExtXMLUtilities::ReadElementFromStream(strstr, encoding);
...
//----------------------------------------------------------------------------
vtkXMLDataElement*
vtkExtXMLUtilities::ReadElementFromFile(const char *filename, int encoding)
{
  if (!filename)
    {
    return NULL;
  ...
  return vtkExtXMLUtilities::ReadElementFromStream(is, encoding);
}

//----------------------------------------------------------------------------
void vtkExtXMLUtilities::ReadElementFromAttributeArray(
...
//----------------------------------------------------------------------------
void vtkExtXMLUtilitiesFindSimilarElementsInternal(
  vtkXMLDataElement *elem, 
  vtkXMLDataElement *tree, 
  vtkExtXMLUtilitiesDataElementContainer *results)
{
  ...
  if (elem->IsEqualTo(tree))
    ...
    for (int i = 0; i < tree->GetNumberOfNestedElements(); i++)
      {
      vtkExtXMLUtilitiesFindSimilarElementsInternal(
...
//----------------------------------------------------------------------------
int vtkExtXMLUtilities::FindSimilarElements(vtkXMLDataElement *elem, 
                                         vtkXMLDataElement *tree, 
                                         vtkXMLDataElement ***results)
{
  ...
  vtkExtXMLUtilitiesDataElementContainer *container = 
    new vtkExtXMLUtilitiesDataElementContainer;
  
  vtkExtXMLUtilitiesFindSimilarElementsInternal(elem, tree, container);
  ...
  if (size)
    ...
    *results = new vtkXMLDataElement* [size];
    ...
    size = 0;
    for (vtkExtXMLUtilitiesDataElementContainer::const_iterator 
...
//----------------------------------------------------------------------------
void vtkExtXMLUtilities::FactorElements(vtkXMLDataElement *tree)
{
  if (!tree)
    {
    return;
  ...
  while (vtkExtXMLUtilities::FactorElementsInternal(tree, tree, pool)) {};
...
//----------------------------------------------------------------------------
int vtkExtXMLUtilities::FactorElementsInternal(vtkXMLDataElement *tree,
                                            ...
                                            vtkXMLDataElement *pool)
{
  ...
  int nb_of_similar_trees = vtkExtXMLUtilities::FindSimilarElements(
  ...
  if (!nb_of_similar_trees)
    ...
    for (int i = 0; i < tree->GetNumberOfNestedElements(); i++)
      {
      res += vtkExtXMLUtilities::FactorElementsInternal(
...
//----------------------------------------------------------------------------
void vtkExtXMLUtilities::UnFactorElements(vtkXMLDataElement *tree)
{
  if (!tree)
    {
    return;
  ...
  vtkExtXMLUtilities::UnFactorElementsInternal(tree, pool);
...
//----------------------------------------------------------------------------
int vtkExtXMLUtilities::UnFactorElementsInternal(vtkXMLDataElement *tree,
                                              vtkXMLDataElement *pool)
{
  if (!tree || !pool)
    {
  ...
  for (int i = 0; i < tree->GetNumberOfNestedElements(); i++)
    {
    res += vtkExtXMLUtilities::UnFactorElementsInternal(
