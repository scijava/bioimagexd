...
class ShapeAnnotation(OGLAnnotation):
	"""
	A base class for shape (rectangle, circle and polygon) annotations.
	"""
	def __init__(self, zoomFactor, parent):
		"""
		Initialization
		"""
		...
		global count
		if self.parent != None:
			self.parent.AddAnnotation(self)
		else:
...
class MyPolygonSketch(OGLAnnotation, ogl.Shape):
	...
	def __init__(self, zoomFactor = 1.0):
		...
		OGLAnnotation.__init__(self, zoomFactor)
		self._isROI = 1
		self.points = []
...
class MyRectangle(ShapeAnnotation, ogl.RectangleShape):

	AnnotationType = "RECTANGLE"
	def __init__(self, w, h, zoomFactor = 1.0, parent = None):
		"""
		Initialization
		"""
		self.name = "Rectangle"
		ShapeAnnotation.__init__(self, zoomFactor, parent)
		ogl.RectangleShape.__init__(self, w, h)
		self._isROI = 1
	...
	def getCoveredPoints(self):
		pts = {}
		fromx, fromy, tox, toy = self.getROICoordinates()
		for x in range(fromx, tox):
			for y in range(fromy, toy):
				pts[(x, y)] = 1
		return pts

class MyCircle(ShapeAnnotation, ogl.CircleShape):

	AnnotationType = "CIRCLE"
	def __init__(self, diam, zoomFactor = 1.0, parent = None):
		"""
		Initialization
		"""
		self.name = "Circle"
	...
	def getFoo(self):
		...
		return [w, h, cx, cy, c, a]

	def getROICoordinates(self):
		w, h, cx, cy = self.getFoo()[:-2]
		return [int(cx - w // 2), int(cy - h // 2), int(cx + w // 2), int(cy + h // 2)]

	def getCoveredPoints(self):
		w, h, cx, cy, c, a = self.getFoo()
...
class MyPolygon(ShapeAnnotation, ogl.PolygonShape):
	...
	def __init__(self, zoomFactor = 1.0, parent = None):
		"""
		Initialization
		"""
		self.name = "Polygon"
...
#		self._originalPoints = self._points[:]
	...
	def getROICoordinates(self):
		return self.getFoo()[:-2]

	def getCoveredPoints(self):
		x0, y0, x1, y1, cx, cy = self.getFoo()
...
class My3DAnnotation():
	...
	def AddAnnotation(self, annotation):
		if annotation.parent != self:
			annotation.parent = self
		self.annotations.append(annotation)
		# Rectangles and circles : .setName sets ._name
		# Polygons : .name sets .name
		# Why there are two I don't know. Polygons (again) handles
		# this differently.
		annotation.setName(self.name + " #%d" % count[self.__class__])
		annotation.name = annotation.getName()
		annotation.sliceNumber = len(self.annotations)
...
class My3DPolygon(My3DAnnotation):
	...
	def __init__(self, polygons = None):
		...
		Initialization
		"""
		self.name = "3D Polygon"
		My3DAnnotation.__init__(self, polygons)

class My3DCircle(My3DAnnotation):
	"""
	3D circle. A list containing MyCircle objects.
	"""
	AnnotationType = "3D_CIRCLE"
	def __init__(self, circles = None):
		"""
		Initialization
		"""
		self.name = "3D Circle"
		My3DAnnotation.__init__(self, circles)

class My3DRectangle(My3DAnnotation):
	"""
	3D rectangle. A list containing MyRectangle objects.
	"""
	AnnotationType = "3D_RECTANGLE"
	def __init__(self, rectangles = None):
		"""
		Initialization
		"""
		self.name = "3D Rectangle"
		My3DAnnotation.__init__(self, rectangles)
