...
class Shape(ShapeEvtHandler):
	...
	def OnDrawContents(self, dc):
		...
		for region in self._regions:
			...
			if not self.GetDisableLabel():
				DrawFormattedText(dc, region.GetFormattedText(), x0 + self._xpos, y0 + self._ypos, bound_x - 2 * self._textMarginX, bound_y - 2 * self._textMarginY, region.GetFormatMode())
	...
	def OnEraseContents(self, dc):
		...
		x0, y0, w, h = self.GetCanvas().GetClientRect()
		dc.DrawRectangle(x0 + topLeftX - penWidth, y0 + topLeftY - penWidth, x0 + maxX + penWidth * 2 + 4, y0 + maxY + penWidth * 2 + 4)
	...
	def OnDrawBranchesAttachment(self, dc, attachment, erase = False):
		...
		# Draw neck
		x0, y0, w, h = self.GetCanvas().GetClientRect()
		dc.DrawLine(x0 + root[0], y0 + root[1], x0 + neck[0], y0 + neck[1])

		if count > 1:
			# Draw shoulder-to-shoulder line
			dc.DrawLine(x0 + shoulder1[0], y0 + shoulder1[1], x0 + shoulder2[0], y0 + shoulder2[1])
		# Draw all the little branches
		for i in range(count):
			pt, stemPt = self.GetBranchingAttachmentPoint(attachment, i)
			dc.DrawLine(x0 + stemPt[0], y0 + stemPt[1], x0 + pt[0], y0 + pt[1])
...
class PolygonShape(Shape):
	...
	def OnDraw(self, dc):
		...
		if self._shadowMode != SHADOW_NONE:
			...
			dc.DrawPolygon(self._points, x0 + self._xpos + self._shadowOffsetX, y0 + self._ypos, self._shadowOffsetY)
		...
		if self._brush:
			dc.SetBrush(self._brush)
		dc.DrawPolygon(self._points, x0 + self._xpos, y0 + self._ypos)
	...
	def OnDrawOutline(self, dc, x, y, w, h):
		...
		dc.DrawPolygon(intPoints, x0 + x, y0 + y)
...
class EllipseShape(Shape):
	...
	def OnDraw(self, dc):
		if self._shadowMode != SHADOW_NONE:
			...
			dc.SetPen(TransparentPen)
			x0, y0, w, h = self.GetCanvas().GetClientRect()
			dc.DrawEllipse(x0 + self._xpos - self.GetWidth() / 2.0 + self._shadowOffsetX,
						   y0 + self._ypos - self.GetHeight() / 2.0 + self._shadowOffsetY,
						   x0 + self.GetWidth(), y0 + self.GetHeight())
		...
		x0, y0, w, h = self.GetCanvas().GetClientRect()
		dc.DrawEllipse(x0 + self._xpos - self.GetWidth() / 2.0, y0 + self._ypos - self.GetHeight() / 2.0, x0 + self.GetWidth(), y0 + self.GetHeight())
